...
def modify_comment(comment_id):
    ...
    if request.method == 'DELETE':
        ...
        if comment.user_id is not None:
            ...
            if not authorized:
                return jsonify({'error': 'Permission denied. You can only modify your own comments.'}), 403
...
def book_lesson():
    ...
    email_body += "Please be ready to host the lesson. Thanks!"
    
    send_notification_email(
        subject=f"New EbiUI Zoom Lesson: {course_name}",
        body=email_body
    )
    
    return jsonify({
        'message': 'Zoom lesson booked and confirmed successfully!',
        'zoom_link': zoom_link,
