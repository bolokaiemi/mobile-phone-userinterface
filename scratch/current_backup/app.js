...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderBookingsUI(bookings) {
        ...
        bookings.forEach(booking => {
            ...
            item.style.cssText = 'background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 8px; padding: 10px; display: flex; flex-direction: column; gap: 6px;';
            item.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <strong style="font-size: 0.8rem; color: white;">${booking.course_name}</strong>
                    <span style="font-size: 0.7rem; color: var(--text-secondary);">${booking.created_at || ''}</span>
                </div>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 4px;">
                    <a href="${booking.zoom_link}" target="_blank" class="btn-primary" style="padding: 4px 8px; font-size: 0.75rem; border-radius: 4px; text-decoration: none; display: inline-flex; align-items: center; gap: 4px;">
                        <i class="fa-solid fa-video"></i> Join Class
                    </a>
                    <span style="font-size: 0.7rem; color: #34d399; font-weight: 500;"><i class="fa-solid fa-circle-check"></i> Paid & Confirmed</span>
                </div>
            `;
    ...
    function renderBookingsOfflineFallback() {
        try {
            ...
            renderBookingsUI(localBookings);
        } catch (e) {
        ...
        courseListUl.addEventListener('change', (e) => {
            if (e.target.classList.contains('live-upgrade-checkbox')) {
                ...
                if (checkbox.checked) {
                    ...
                    if (labelSpan) {
                        labelSpan.innerHTML = `<i class="fa-solid fa-chalkboard-user"></i> Meet AB direct for a live section. This is where you will book for lesson and you will pay before you enter the class.`;
    ...
    if (premiumPaymentForm) {
        premiumPaymentForm.addEventListener('submit', async (e) => {
            ...
            if (serverSuccess) {
                if (!currentUser && billingEmailVal) {
                    try {
                        ...
                        localBookings.push({
                            course_name: courseName,
                            zoom_link: generatedZoomLink || 'https://zoom.us',
                    ...
                    showNotification(`Lesson booked! Check your email (${billingEmailVal}) for Zoom details.`);
                    alert(`Thank you, ${billingNameVal}!\n\nYour lesson for "${courseName}" has been successfully booked.\n\nZoom Link: ${generatedZoomLink || 'Check your email'}\n\nA confirmation email has been sent to ${billingEmailVal}.`);
                } else {
                    showNotification('Lesson booked successfully!');
                    alert(`Congratulations!\n\nYour live lesson for "${courseName}" has been booked.\n\nZoom Link: ${generatedZoomLink || 'Check your dashboard'}\n\nYou can access it from your dashboard.`);
                    loadBookings();
                }
            } else {
                // Offline booking fallback
                const offlineZoomLink = `https://zoom.us/j/${Math.floor(1000000000 + Math.random() * 9000000000)}?pwd=offline`;
                try {
                    const localBookings = JSON.parse(localStorage.getItem('ebiui_local_bookings') || '[]');
                    localBookings.push({
                        course_name: courseName,
                        zoom_link: offlineZoomLink,
                        created_at: new Date().toISOString().slice(0, 19).replace('T', ' ')
                    });
                    localStorage.setItem('ebiui_local_bookings', JSON.stringify(localBookings));
                } catch (e) {}

                showNotification('Offline mode: Saved booking locally.');
                alert(`Offline Checkout Completed (Demo)!\n\nCourse: ${courseName}\nBilling Name: ${billingNameVal}\nEmail: ${billingEmailVal || 'None'}\n\nZoom Link: ${offlineZoomLink}\n\nSaved locally in your browser cache.`);
            ...
            if (activeUpgradeCheckbox) {
                activeUpgradeCheckbox.checked = true;
                activeUpgradeCheckbox.disabled = true;
                const labelSpan = activeUpgradeCheckbox.nextElementSibling;
                if (labelSpan) {
                    labelSpan.innerHTML = `<i class="fa-solid fa-circle-check"></i> Direct Session Active! ✔`;
        ...
        if (courseListUl) {
            ...
            coursesData.forEach(course => {
                ...
                const checkboxHtml = isBooked 
                    ? `<input type="checkbox" class="live-upgrade-checkbox" data-course="${course.title}" checked disabled>`
                    : `<input type="checkbox" class="live-upgrade-checkbox" data-course="${course.title}">`;
                    
                const spanHtml = isBooked
                    ? `<span class="live-upgrade-span" style="color: #10b981;"><i class="fa-solid fa-circle-check"></i> Direct Session Active! ✔</span>`
                    : `<span class="live-upgrade-span"><i class="fa-solid fa-chalkboard-user"></i> Meet AB direct for a live section. This is where you will book for lesson and you will pay before you enter the class.</span>`;
                    ...
                    <div class="course-main-row">
                        ...
                        <div class="course-info">
                            <a href="${course.w3_link || '#'}" target="_blank" class="course-name-link" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center; gap: 4px;">
                                <span>${course.title}</span> <i class="fa-solid fa-arrow-up-right-from-square" style="font-size: 0.75rem; opacity: 0.6;"></i>
                            </a>
